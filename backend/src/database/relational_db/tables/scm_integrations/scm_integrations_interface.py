...
class ScmIntegrationInterface:
    ...
    async def list_all(self) -> list[ScmIntegration]:
        rows = await self.session.scalars(select(ScmIntegration))
    ...
    async def get_by_id(self, integration_id: UUID) -> ScmIntegration | None:
        return await self.session.scalar(
            select(ScmIntegration).where(ScmIntegration.id == integration_id)
        )
